...
class OrdersController extends VoyagerBaseController
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $statuses = Status::withTranslations()->get()->toArray();
        $user = User::where('id', $order->created_by)->first();
        $admin = Auth::user();
        $address = Address::where('id', $order->address_id)->first();
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'items', 'address', 'user', 'statuses', 'order', 'admin'));
    }
